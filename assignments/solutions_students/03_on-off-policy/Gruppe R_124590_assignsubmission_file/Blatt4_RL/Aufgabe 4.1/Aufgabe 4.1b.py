...
env = gym.make("gymnasium_env/GridWorld")
env.reset()
...
def TD(n):
    action_values = [[[0 for _ in range(4)] for _ in range(5)] for _ in range(5)]
    rewardPerEpisode = []
    for k in range(episodes):
        ...
        while not done:
            ...
            for j in range(0,n+2):
                G += np.pow(gamma,j) *rewards[-n-2+j]
            action_values[x][y][action] = action_values[x][y][action] + alpha* (G - action_values[x][y][action])

            
        rewardPerEpisode.append(sum(rewards))
    return action_values, rewardPerEpisode
...
c = 1
action_values ,rewardPerEpisode = TD(c)
...
for k in range(4):
    ...
    for i in range(5):
        row = []
        for j in range(5):
            row.append(action_values[i][j][k])
        matrix.append(row)
    actValues.append(matrix)

for k in range(4):
    sns.heatmap(actValues[k], annot=True, cmap="YlGnBu" , cbar=True)
    plt.title(f"Heatmap of action {k} | TD({c})")
    plt.show()
...
plt.xlabel("Episodes")
plt.ylabel("Reward")
plt.title(f"Reward per Episode | TD({c})")
plt.show()
...
for k in steps:
    act_values , rewardPE = TD(k)
    rewardCum = np.cumsum(rewardPE)
    plt.plot(rewardCum, label =f"TD({k})")
