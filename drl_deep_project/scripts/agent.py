        ...
        for i in range(danger_map.shape[0]):
            for j in range(danger_map.shape[1]):
                if danger_map[i, j] == -4:
                    ...
                    for k in range(1, 4):
                        if i + k < danger_map.shape[0] and danger_map[i + k, j] != -10 and not wall_hit_up:
                            danger_map[i + k, j] = -1
                        if i + k < danger_map.shape[0] and danger_map[i + k, j] == -10:
                            wall_hit_up = True
                        if j + k < danger_map.shape[1] and danger_map[i, j + k] != -10 and not wall_hit_right:
                            danger_map[i, j + k] = -1
                        if j + k < danger_map.shape[1] and danger_map[i, j + k] == -10:
                            wall_hit_right = True
                        if i - k >= 0 and danger_map[i - k, j] != -10 and not wall_hit_down:
                            danger_map[i - k, j] = -1
                        if i - k >= 0 and danger_map[i - k, j] == -10:
                            wall_hit_down = True
                        if j - k >= 0 and danger_map[i, j - k] != -10 and not wall_hit_left:
                            danger_map[i, j - k] = -1
