...
class Model():
    def __init__(self, load=True, path=Path(__file__).parent / "model.pt", weights_suffix=None):
        ...
        self.eps_decay = 100000 # self.eps_decay controls the rate of exponential decay of epsilon, higher means a slower decay
    ...
    def load_weights(self, suffix=None):
        ...
        try:
            ...
            self.policy_net.load_state_dict(torch.load(model_path, weights_only=True, map_location=device))
            self.target_net.load_state_dict(torch.load(model_path, weights_only=True, map_location=device))
